...
1. Mục tiêu: Phân loại 10 lớp đối tượng (máy bay, ô tô, chim, mèo,...).
2. Kiến trúc: Sử dụng 3 khối (Block) tích chập với số kênh tăng dần (64 -> 128 -> 256).
- Code chính :
```python
...
3. Kỹ thuật: RandomCrop và RandomRotation để tăng cường dữ liệu nhỏ (32x32).
```python
...
```
5. Mô hình được huấn luyện trong 100 Epoch gồm các bước :
...
#### Với dataset Cats vs Dogs
1. Mục tiêu: Phân loại nhị phân giữa chó và mèo.
2. Tiền xử lý: Ảnh được đưa về kích thước 128x128 để giữ chi tiết tốt hơn so với CIFAR.
- Code chính :
```python
transform_train = transforms.Compose([
...
scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', patience=5, factor=0.5)
```
5. Quy trình Huấn luyện Cat vs Dog (50 Epochs)
...
#### Với dataset PlantVillage
1. Mục tiêu: Nhận diện các loại bệnh trên lá cây trồng.
2. Quy mô: Phân loại đa lớp với số lượng lớp lớn hơn (tùy thuộc vào dữ liệu trong thư mục color).
3. Tối ưu: Sử dụng cấu trúc conv_layer đóng gói sẵn (Conv + BatchNorm + ReLU + MaxPool) để mã nguồn sạch sẽ và hiệu quả.
```python
...
5. Quy trình Huấn luyện PlantVillage (40 Epochs): 
